/** scanner/rules/tag.mjs — 标签/状态规则：R009 R010 R012 */
import { lineOf, issue, findTags } from "./_shared.mjs";

/** 状态类字段名关键字（动态流转状态） */
const STATUS_FIELD_PATTERN = /(?:Status|State|Flag|status|state|flag)(?=\s*["']|:\s*["']|$)/;

/** 分类字段名关键字（组织/归档属性） */
const CLASSIFY_FIELD_PATTERN = /(?:Type|Level|Kind|Class|Category|type|level|kind|class|category)(?=\s*["']|:\s*["']|$)/;
  ...
  // R009: 状态字段纯文本渲染（应使用 renderTagNode / ElTag）
  {
    ...
    check(template, file, lineOffset) {
      ...
      for (const tag of findTags(template, "el-table-column")) {
        ...
        if (!STATUS_FIELD_PATTERN.test(fieldName) && !CLASSIFY_FIELD_PATTERN.test(fieldName)) continue;
        // 有渲染器则跳过
        if (
          /renderTagNode|renderOps|ElTag|jh-tag|defaultSlot|defaultNode|renderClassifyTag|renderBadge/.test(tag.text)
        )
          continue;
        issues.push(
          issue(
  ...
  // R010: 分类字段使用填充色 ElTag（应使用 effect="plain"）
  {
    ...
    check(template, file, lineOffset) {
      ...
      for (const tag of findTags(template, "el-tag")) {
        ...
        const context = template.slice(contextStart, tag.index + tag.text.length);
        ...
        if (/effect\s*=\s*["']dark["']/.test(tag.text) || !/effect\s*=/.test(tag.text)) {
          issues.push(
            issue(
              file,
              lineOf(template, tag.index, lineOffset),
  ...
  // R012: 弹窗内 el-table 缺少空状态
  {
    ...
    check(template, file, lineOffset) {
      ...
      while ((dm = dialogPattern.exec(template)) !== null) {
        ...
        const dialogEnd = dialogStart + dialogCloseMatch.index + '</el-dialog>'.length;
